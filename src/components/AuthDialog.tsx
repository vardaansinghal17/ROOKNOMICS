...
export default function AuthDialog({ open, onClose }: AuthDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (mode === 'signup') {
      // Show OTP verification step instead of closing
      setShowOtp(true);
    } else {
      // Login — just close (mock)
      onClose();
    }
  };

  const handleOtpVerified = () => {
    // OTP passed — close both dialogs and reset
    setShowOtp(false);
    setEmail('');
    setPassword('');
    setName('');
  ...
  const handleOtpClose = () => {
    // User dismissed OTP; keep AuthDialog open so they can retry
    setShowOtp(false);
  };

  return (
    <>
      {/* Main auth dialog — hidden behind OTP when OTP is showing */}
      {!showOtp && (
        <>
      ...
      {/* OTP dialog — only shown after signup form submit */}
      <OtpDialog
        open={showOtp}
        email={email}
